...
function CustomTitleBar(button_pos)
  ...
  if im.BeginPopup(ctx, "Settings", im.WindowFlags_NoMove) then
    ...
    -- Layout selector (use cached layout ID, zero function calls)
    if layouts_data and layouts_data.layouts then
      im.Separator(ctx)
      im.Text(ctx, "Device Layout:")
      
      local available_layouts = LayoutManager_GetAvailableLayouts(layouts_data)
